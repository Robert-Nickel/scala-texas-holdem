  ...
  "A Table" when {
    ...
    "given players" should {
      ...
      "fold for the current player in" in {
        ...
        val playerWithoutHoleCards = Seq(newTable.players(currentPlayer).copy(holeCards = (None, None)))
    ...
    "given currentPlayer is not in the round" should {
      val table = Table(List(
        Player("Bob", 200, (None, None)),
        Player("Ali", 200, (Option.apply(Card('A', 'h')), Option.apply(Card('A', 's'))))))
